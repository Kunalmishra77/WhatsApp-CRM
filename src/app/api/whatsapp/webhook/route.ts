...
import { findMatchingEnvVerifyToken } from '@/lib/whatsapp/webhook-verify-token'
...
export async function GET(request: Request) {
  try {
    ...
    const envTokenMatch = findMatchingEnvVerifyToken(verifyToken)
    if (envTokenMatch) {
      console.log(
        `[webhook] Meta verification succeeded via ${envTokenMatch.key}`,
      )
